...
class BinaryTreesV3
{
    ...
    public static void Run(string[] args)
    {
        ...
        var tasks = new Task<int>[NoTasks * results.Length];
        var taskIndex = 0;
        ...
        for (int i = 0; i < results.Length; i++)
        {
            var check = tasks[taskIndex++].Result;
            for (int t = 1; t < NoTasks; t++)
                check += tasks[taskIndex++].Result;
            results[i] ="\t check: " + check; //(n * NoTasks) + "\t trees of depth " + depth +
